...
SecurityContext.prototype.requestTokenForClient = function(serviceCredentials, scopes, cb) {
    ...
    if (this.checkScope('uaa.user') === false) {
        var error = new Error('JWT token does not include scope "uaa.user".');
        return cb(error, null);
    }
    // user token flow
    var options = {
        url : serviceCredentials.url + '/oauth/token?grant_type=user_token&response_type=token&client_id=' + serviceCredentials.clientid,
    ...
    request.post(
        ...
        function(error, response, body) {
            ...
            if (response.statusCode === 401) {
                debugTrace('getTokenForClient: Call to /oauth/token was not successful (grant_type: user_token). Bearer token invalid, requesting client does not have grant_type=user_token or no scopes were granted.');
            ...
            if (response.statusCode !== 200) {
                debugTrace('getTokenForClient: Call to /oauth/token was not successful (grant_type: user_token). HTTP status code: ' + response.statusCode);
            ...
            // refresh token flow
            options = {
                url : serviceCredentials.url + '/oauth/token?grant_type=refresh_token&refresh_token=' + json.refresh_token,
                headers: { Accept: 'application/json' },
                auth: {
            ...
            request.post(
                    ...
                    function(error, response, body) {
                        ...
                        if (response.statusCode === 401) {
                            debugTrace('getTokenForClient: Call to /oauth/token was not successful (grant_type: refresh_token). Client credentials invalid.');
                            return cb(new Error('Call to /oauth/token was not successful (grant_type: refresh_token). Client credentials invalid.'), null);
                        }
                        if (response.statusCode !== 200) {
                            debugTrace('getTokenForClient: Call to /oauth/token was not successful (grant_type: refresh_token). HTTP status code: ' + response.statusCode);
