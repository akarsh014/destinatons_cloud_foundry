...
function addOauthHeader(headers, tokens, route, destination) {
  if (route && route.destination) { // Destination flow
    ...
    if (serviceUtils.getGrantType(route.credentials) !== 'user_token') {// html5 apps repo flow
...
function addAppHostIdHeader(businessServiceTag, headers) {
  var businessServiceCredentials = serviceUtils.getServiceCredentials(businessServiceTag);
