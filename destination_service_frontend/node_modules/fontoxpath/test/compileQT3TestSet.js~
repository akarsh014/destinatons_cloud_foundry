...
const shouldRunTestByName = fs.readFileSync('test/runnableTestSets.csv', 'utf8')
	.split('\n')
	.map(line=>line.split(','))
	.reduce((accum, [name, run]) => Object.assign(accum, { [name]: run === 'true' }), Object.create(null));

const unrunnableTestCasesByName = fs.readFileSync('test/unrunnableTestCases.csv', 'utf-8')
	.split('\n')
	.map(line => line.split(','))
...
function getAsserterForTest (baseUrl, testCase, language) {
	console.log('adding test', evaluateXPathToString('@name', testCase));
