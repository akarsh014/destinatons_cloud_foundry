			...
			firstValues => {
				...
				return secondValueSequence.mapAll(
					secondValues => {
						...
						const valueA = firstValues[0];
						const valueB = secondValues[0];
						const typingKey = `${valueA.type}~${valueB.type}~${this._operator}`;
						let prefabOperator = operatorsByTypeingKey[typingKey];
						if (!prefabOperator) {
							prefabOperator = operatorsByTypeingKey[typingKey] = generateBinaryOperatorFunction(this._operator, valueA.type, valueB.type);
						}

						return Sequence.singleton(prefabOperator(valueA, valueB));
					});
			});
	}
}
